...
STOP_WORDS = set()
with open(get_default_stop_words_file(), 'r') as f:
    for line in f:
...
class WordSegmentation(object):
    ...
    def __init__(self, stop_words=None, allow_speech_tags=util.allow_speech_tags):
        """
        Keyword arguments:
        stop_words    -- 用户传入的停用词集合，类型为set，则使用默认的停止词
    ...
    def segment(self, text, lower=False, use_stop_words=True, use_speech_tags_filter=True):
        ...
        # 去除特殊符号
        word_list = [w.word for w in jieba_result if w.flag != 'x']
        ...
        if use_stop_words:
            word_list = [word
                         for word in word_list if word not in self.stop_words]
